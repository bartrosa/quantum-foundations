...
def test_e4_smoke() -> None:
    result = e4_conway_aut_scaling.run(n_workers=1, seeds=1, ns=(10,))
    assert len(result.rows) == 1


def test_e5_smoke() -> None:
    result = e5_outcome_distribution.run(n_workers=1, seeds=1, ns=(10,))
    assert len(result.rows) == 1


def test_e6_smoke() -> None:
    result = e6_hierarchy_collapse.run(n_workers=1, seeds=1, ns=(10,))
    assert len(result.rows) == 1
